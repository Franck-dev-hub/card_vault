...
import { addToCollection, getCollection } from "../../api/collection";
...
export default function CardDetails({ card, onClose, onOwnershipChange }) {
  ...
  // On mount, fetch the user's collection and pre-fill counters for this card.
  useEffect(() => {
    getCollection()
      .then((collection) => {
        const cardId = card.card_id || card.id;
        // Keep only entries that match this specific card.
        const entries = collection.filter((e) => e.card.card_id === cardId);
        if (entries.length === 0) return;

        // Build a quantities object from the real data.
        // e.card.variant comes as lowercase ("normal") so we capitalize
        // the first letter to match the variant keys ("Normal").
        const fromApi = Object.fromEntries(
          entries.map((e) => [
            e.card.variant.charAt(0).toUpperCase() + e.card.variant.slice(1),
            e.quantity,
          ])
        );
        setQuantities((prev) => ({ ...prev, ...fromApi }));
      })
